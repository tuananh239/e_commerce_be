...
class PetitionRepository(BaseRepository, metaclass=Singleton):
    """
        Class này triển khai các phương thức phục vụ cho việc truy vấn dữ liệu trong bảng petition
    """
    def __init__(self) -> None:
        ...
        self.__collection = self.__connection.petition


    def create(self, petition_data: PetitionEntity):
        _response = self.__collection.insert_one(jsonable_encoder(petition_data))
        ...
        _response = PetitionEntity(**{**_response, "id": str(_response["_id"])})
    ...
    def get(self, sort=Sorting(), filter=Filtering(), pagination=Pagination(), search: str=""):
        ...
        if _response:
            _response = [PetitionEntity(**{**petition, "id": str(petition["_id"])}) for petition in _response]
    ...
    def get_detail(self, petition_id: str):
        _response = self.__collection.find_one({'_id': ObjectId(petition_id), "is_active": True})

        if _response:
            return PetitionEntity(**{**_response, "id": str(_response["_id"])})
        else:
            return None
        
    
    def update(self, petition_id: str, petition_data: PetitionEntity):
        self.__collection.update_one(
            {
                "_id": ObjectId(petition_id)
            },
            {
                Syntax.SET: jsonable_encoder(petition_data, exclude_none=True)
            }
        )
